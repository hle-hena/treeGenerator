...
/*   Updated: 2025/09/05 12:11:20 by hle-hena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Grid::placeChar(Vec2 &pos, char toPrint, bool isBranch)
{
	if (pos.x < 0 || pos.x >= width - 1 || pos.y < 0 || pos.y >= height - 1)
		return ;
	if (isBranch && _grid[static_cast<int>(round(pos.y))]
		[static_cast<int>(round(pos.x))] != ' ' && randIntRange(0, 3))
		return ;
	_grid[static_cast<int>(round(pos.y))]
		[static_cast<int>(round(pos.x))] = toPrint;
}

void	Grid::drawLow(Vec2 start, Vec2 end,
...
void	Grid::drawHigh(Vec2 start, Vec2 end,
	const std::string &charSet, char branch)
{
	Vec2	pos(0, 0);
	int		dx = end.x - start.x;
...
void	Grid::drawLine(Vec2 start, Vec2 end, const std::string &charSet)
{
	char	special_char;
	...
	{
		if (start.x > end.x)
			drawLow(end, start, charSet, '\\');
		else
			drawLow(start, end, charSet, '/');
	}
	else
	{
		special_char = '|';
		if (start.y > end.y)
			drawHigh(end, start, charSet, special_char);
		else
			drawHigh(start, end, charSet, special_char);
	}
}
